# ======================================================================
# SSC UNIVERSAL ASSESSMENT APP (V8 - CLIENT SUBMIT MODE)
# - Config-driven routing (JSON-based)
# - Client self-assessment only (no consultant tab)
# - "Submit Assessment" button:
#       -> Runs AI pre-scoring per dimension
#       -> Builds Q&A record
...
# ===========================
st.set_page_config(
    page_title="SSC Universal Assessment App (V8)",
    layout="wide"
)
...
def send_assessment_to_ssc(
    ...
    # === Attachment 2: AI scoring CSV (if available) ===
...
def render_sidebar(cfg: dict):
    ...
    # Optionally still keep weights for SSC internal usage (not visible to client)
    st.sidebar.markdown("---")
...
def render_client_assessment():
    ...
    # Single SUBMIT button – runs AI + sends email to SSC
...
def main():
    ...
    with logo_cols[1]:
        title_html = (
            ...
            "SSC Universal Assessment App (V8)</h1>"
        )
        subtitle_html = (
            "<h4 style='text-align:center;color:gray;font-family:Segoe UI, sans-serif;'>"
            "Client Self-Assessment • AI Pre-Scoring • Secure Submission to StrategyStack</h4>"
        )
    ...
    # Sidebar (template loading, routing info, weights)
    render_sidebar(cfg)

    # Only CLIENT assessment (no consultant tab)
    ...
    st.markdown(
        ...
        SSC Universal Assessment App (V8) - StrategyStack Consulting 2025
        </div>
        """,
        unsafe_allow_html=True,
    )
