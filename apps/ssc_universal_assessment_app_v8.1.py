# ======================================================================
# SSC UNIVERSAL ASSESSMENT APP (V8.1.1 - INDUSTRY + ROLE-AWARE + final_score)
...
#       -> Computes final_score = ai_score * weight
#       -> Builds Q&A record
...
# ===========================
st.set_page_config(
    page_title="SSC Universal Assessment App (V8.1.1)",
    layout="wide"
)
...
def get_industry_context_text(industry_name, dimension, ctx_df):
    ...
    try:
        subset = ctx_df[ctx_df["industry"].str.lower() == str(industry_name).lower()]
        if subset.empty:
            # Try any 'All' / blank industry rows for that dimension
            dim_rows = ctx_df[ctx_df["dimension"].str.lower() == str(dimension).lower()]
            if dim_rows.empty:
                return (
            ...
            dim_rows = subset[subset["dimension"].str.lower() == str(dimension).lower()]
...
# ===========================
# AI PRE-SCORING (INDUSTRY + ROLE-AWARE + final_score)
# ===========================
...
def main():
    ...
    with logo_cols[1]:
        title_html = (
            ...
            "SSC Universal Assessment App (V8.1.1)</h1>"
        )
        subtitle_html = (
    ...
    st.markdown(
        ...
        SSC Universal Assessment App (V8.1.1) - StrategyStack Consulting 2025
        </div>
        """,
        unsafe_allow_html=True,
    )
